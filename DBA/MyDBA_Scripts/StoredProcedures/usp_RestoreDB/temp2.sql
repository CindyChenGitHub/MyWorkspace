...
BEGIN
	...
	-- If DB not exsit and backup not exist, exit with statue 1.
	IF @srcDbExsit = 0 AND @backupExsit = 0
		BEGIN
			...
			return
		END
	-- If DB exsit (and backup not exsit), backup.
	IF @srcDbExsit <> 0
		BEGIN
			IF @backupExsit = 0
				SET @sql = 'BACKUP DATABASE ' + @srcDB  + ' TO DISK=''' + @bkupPath + '''
    WITH COMPRESSION'
			-- If DB exsit and backup exsit, backup as the parameter set
			ELSE
				SET @sql = 'BACKUP DATABASE ' + @srcDB  + ' TO DISK=''' + @bkupPath + '''
    WITH COMPRESSION WITH INIT'
	...
	-- If target exsit and no overwrite, exit with statue 3
	IF @overwrite = 0 and (@targetDbExsit + @targetDataExsit + @targetLogExsit) <> 0
					...
					IF @targetDbExsit <> 0
						BEGIN
							...
							ELSE
								...
								END
						END
					IF @targetDataExsit <> 0
						Set @sql = @sql + ' WITH MOVE'
					IF @targetLogExsit <> 0
						Set @sql = @sql + ' WITH MOVE'
